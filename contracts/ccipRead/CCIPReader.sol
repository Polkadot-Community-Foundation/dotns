...
contract CCIPReader {
        ...
        view
    {
        ...
        (bool ok, bytes memory v) = safeCall(detectEIP140(target), target, call);
        ...
        if (callbackFunction != IDENTITY_FUNCTION) {
            ...
            (ok, v) =
                address(this).staticcall(abi.encodeWithSelector(callbackFunction, v, extraData));
        }
    ...
    function ccipReadCallback(bytes memory response, bytes memory extraData) external view {
        ...
        ccipRead(
            ctx.target,
            abi.encodeWithSelector(ctx.callbackFunction, response, ctx.extraData),
    ...
    function decodeOffchainLookup(bytes memory v) internal pure returns (EIP3668.Params memory p) {
