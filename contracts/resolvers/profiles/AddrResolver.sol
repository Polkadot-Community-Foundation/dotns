    ...
    ResolverBase
{
        ...
        authorised(node)
    {
        ...
        versionable_addresses[recordVersions[node]][node][coinType] = addressBytes;
    }
    ...
    {
        mapping(uint256 => bytes) storage addrs = versionable_addresses[recordVersions[node]][node];
    ...
    function hasAddr(bytes32 node, uint256 coinType) external view returns (bool) {
        return versionable_addresses[recordVersions[node]][node][coinType].length > 0;
    }

    /// @inheritdoc IERC165
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return type(IAddrResolver).interfaceId == interfaceId
