...
contract OffchainDNSResolver is IExtendedResolver, IERC165 {
    ...
    function resolveCallback(
        ...
        returns (bytes memory)
    {
        (bytes memory name, bytes memory query, bytes4 selector) =
            abi.decode(extraData, (bytes, bytes, bytes4));
        ...
        for (RRUtils.RRIterator memory iter = data.iterateRRs(0); !iter.done(); iter.next()) {
            ...
            (address dnsresolver, bytes memory context) =
                parseRR(iter.data, iter.rdataOffset, iter.nextOffset);
