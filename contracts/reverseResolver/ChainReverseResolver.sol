...
contract ChainReverseResolver is AbstractReverseResolver, GatewayFetchTarget, Ownable {
    ...
    function resolveNames(address[] memory addrs) external view returns (string[] memory) {
        if (addrs.length > 255) {
            revert RequestOverflow();
        }
        GatewayRequest memory req = GatewayFetcher.newRequest(uint8(addrs.length));
        req.setTarget(l2Registrar); // target L2 registrar
