...
contract ETHReverseResolver is AbstractReverseResolver {
    ...
    function _resolveName(address addr) internal view override returns (string memory name) {
        name = addrRegistrar.nameForAddr(addr);
        if (bytes(name).length == 0) {
            bytes32 node = keccak256(
                abi.encode(ADDR_REVERSE_NODE, keccak256(bytes(HexUtils.addressToHex(addr))))
            );
            address resolver = registry.resolver(node);
            (bool ok, bytes memory v) =
                resolver.staticcall{gas: 100000}(abi.encodeCall(INameResolver.name, (node)));
            if (ok && v.length >= 32) {
    ...
    function resolveNames(address[] memory addrs) external view returns (string[] memory names) {
