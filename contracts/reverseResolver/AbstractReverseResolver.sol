...
abstract contract AbstractReverseResolver is IExtendedResolver, INameReverser, ERC165 {
    ...
    function _resolveName(address addr) internal view virtual returns (string memory name);
        ...
        returns (bytes memory result)
    {
        ...
        if (selector == INameResolver.name.selector) {
            ...
            if (
                a.length != 20
                    || !(coinType == COIN_TYPE_DEFAULT ? ENSIP19.isEVMCoinType(ct) : ct == coinType)
            ) {
            ...
            return abi.encode(coinType == COIN_TYPE_ETH ? registrar : address(0));
        } else if (selector == IAddressResolver.addr.selector) {
            (bool valid,) = ENSIP19.parseNamespace(name, 0);
            if (!valid) revert UnreachableName(name);
            (, uint256 ct) = abi.decode(data[4:], (bytes32, uint256));
            return abi.encode(coinType == ct ? abi.encodePacked(registrar) : new bytes(0));
        } else {
