...
contract TestRRUtils {
    ...
    function testLabelCount() public pure {
        ...
        require(
            hex"066574686c61620378797a00".labelCount(6 + 1) == 1, "nameLength('(bthlab).xyz.') == 6"
        );
    }
    ...
    function testIterateRRs() public pure {
        ...
        for (RRUtils.RRIterator memory iter = rrs.iterateRRs(0); !iter.done(); iter.next()) {
            require(uint256(iter.dnstype) == 1, "Type matches");
            require(uint256(iter.class) == 1, "Class matches");
            require(uint256(iter.ttl) == 3600, "TTL matches");
    ...
    function testCompareNames() public pure {
        ...
        bytes memory verylong1_eth =
            hex"223031323334353637383930313233343536373839303132333435363738393031613031303132333435363738393031323334353637383930313233343536373839303132333435363738393031323334353637380365746800";
        bytes memory verylong2_eth =
            hex"2130313233343536373839303132333435363738393031323334353637383930316131303132333435363738393031323334353637383930313233343536373839303132333435363738393031323334353637380365746800";

        require(
            hex"0301616100".compareNames(hex"0302616200") < 0, "label lengths are correctly checked"
        );
        require(
