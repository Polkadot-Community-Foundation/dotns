...
library BytesUtils {
    ...
    function keccak(bytes memory v, uint256 off, uint256 len) internal pure returns (bytes32 ret) {
    ...
    function compare(bytes memory vA, bytes memory vB) internal pure returns (int256) {
        ...
        returns (bool)
    {
        ...
        _checkBound(vB, offB);
        unchecked {
            return keccak(vA, offA, vA.length - offA) == keccak(vB, offB, vB.length - offB);
        }
    }
    ...
    /// @param vB The second bytes.
    /// @return True if the byte ranges are equal.
    function equals(bytes memory vA, uint256 offA, bytes memory vB) internal pure returns (bool) {
        return vA.length == offA + vB.length && keccak(vA, offA, vB.length) == keccak256(vB);
    }
    ...
    function equals(bytes memory vA, bytes memory vB) internal pure returns (bool) {
    ...
    function readUint8(bytes memory v, uint256 off) internal pure returns (uint8) {
        _checkBound(v, off + 1);
        unchecked {
            return uint8(v[off]);
        }
    ...
    function readUint16(bytes memory v, uint256 off) internal pure returns (uint16 ret) {
        _checkBound(v, off + 2);
        assembly ("memory-safe") {
    ...
    function readUint32(bytes memory v, uint256 off) internal pure returns (uint32 ret) {
        _checkBound(v, off + 4);
        assembly ("memory-safe") {
    ...
    function readBytes20(bytes memory v, uint256 off) internal pure returns (bytes20 ret) {
    ...
    function readBytes32(bytes memory v, uint256 off) internal pure returns (bytes32 ret) {
        ...
        pure
    {
        ...
        unchecked {
            LibMem.copy(LibMem.ptr(vDst) + offDst, LibMem.ptr(vSrc) + offSrc, len);
        }
    }
