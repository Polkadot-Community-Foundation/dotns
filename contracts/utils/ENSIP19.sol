...
library ENSIP19 {
    ...
    function parse(bytes memory name)
        internal
        pure
        returns (bytes memory addressBytes, uint256 coinType)
    {
    ...
    function parseNamespace(
        ...
        internal
        pure
        returns (bool valid, uint256 coinType)
    {
        (bytes32 labelHash, uint256 offsetTLD) = NameCoder.readLabel(name, offset);
        if (labelHash == keccak256(bytes(SLUG_ETH))) {
            ...
            (bytes32 word, bool validHex) = HexUtils.hexStringToBytes32(name, 1 + offset, offsetTLD);
