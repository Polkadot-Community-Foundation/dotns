...
abstract contract ResolverCaller is CCIPBatcher {
        ...
        returns (bytes memory)
    {
        ...
        bool extended = ERC165Checker.supportsERC165InterfaceUnchecked(
            resolver, type(IExtendedResolver).interfaceId
        );
        if (
            ...
            if (extended) {
                ...
                ccipRead(resolver, abi.encodeCall(IExtendedResolver.resolve, (name, data)));
            } else {
                ccipRead(
                    resolver,
                    data,
        ...
        bytes[] memory calls;
        if (multi) {
            calls = abi.decode(BytesUtils.substring(data, 4, data.length - 4), (bytes[]));
        } else {
            calls = new bytes[](1);
            calls[0] = data;
        }
        ...
        ccipRead(
            address(this),
            abi.encodeCall(this.ccipBatch, (createBatch(resolver, calls, batchGateways))),
