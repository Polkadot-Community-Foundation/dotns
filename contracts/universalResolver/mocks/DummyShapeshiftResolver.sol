...
contract DummyShapeshiftResolver is IExtendedResolver, IERC165, IERC7996 {
    ...
    function getResponse(bytes memory call) public view returns (bytes memory) {
        if (deriveMulticall && bytes4(call) == IMulticallable.multicall.selector) {
            bytes[] memory m = abi.decode(BytesUtils.substring(call, 4, call.length - 4), (bytes[]));
    ...
    function supportsInterface(bytes4 x) external view returns (bool) {
        ...
        return type(IERC165).interfaceId == x
    ...
    function resolve(bytes memory, bytes memory call) external view returns (bytes memory) {
    ...
    function _revertOffchain(bytes memory v) internal view {
        ...
        urls[0] = revertURL;
        revert OffchainLookup(address(this), urls, "", this.callback.selector, v);
    }

    function callback(bytes memory, bytes memory v) external view returns (bytes memory) {
        _revertIfError(v);
