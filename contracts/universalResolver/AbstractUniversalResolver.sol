...
abstract contract AbstractUniversalResolver is IUniversalResolver, CCIPBatcher, ERC165 {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return type(IUniversalResolver).interfaceId == interfaceId
    ...
    {
        return reverseWithGateways(lookupAddress, coinType, batchGatewayProvider.gateways());
    }
        ...
        returns (string memory primary, address resolver, address reverseResolver)
    {
        ...
        _callResolver(
            ...
            abi.encode(ReverseArgs(lookupAddress, coinType, gateways, info.resolver))
        );
    }
        ...
        view
    {
        ...
        if (
            ...
            ccipRead(
                ...
                info.extended ? abi.encodeCall(IExtendedResolver.resolve, (info.name, call)) : call,
                this.resolveDirectCallback.selector,
                this.resolveDirectCallbackError.selector,
                abi.encode(info.extended, bytes4(call), callbackFunction, extraData)
            );
        }
        bytes[] memory calls;
        if (multi) {
