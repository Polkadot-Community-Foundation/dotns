...
To request a grant, open a Whitelist Request issue in this repository. The issue is labelled whitelist-request by the template and must include the address, address type, target network (Paseo V2 or Paseo Review), and a clear description of why the PoP bypass is needed. A maintainer approves by applying whitelist-approved. An operator then issues the grant with `grantName`, as described in [Name grants](./DEPLOYMENTS.md#name-grants-whitelisting); this step is manual while the request template has no label field, so state the label you want in the description. A grant does not register a name or bypass ownership rules; it only permits the named address to register that one label without a PoP status.

For the operator-side mechanics, granting the operator role, granting it to many operators, and granting one or many names through the dotNS SDK CLI, the hosted app, or `cast`, see the [Name grants](./DEPLOYMENTS.md#name-grants-whitelisting) section in DEPLOYMENTS.md.
