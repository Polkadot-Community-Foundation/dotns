...
OUT_FILE="${DEPLOY_DIR}/ens_stack.json"
...
append_json() {
  local n=$1 a=$2
  tmp=$(mktemp)
  jq --arg n "$n" --arg a "$a" '. + {($n): $a}' "$OUT_FILE" > "$tmp" && mv "$tmp" "$OUT_FILE"
}
...
deploy() {
  ...
  [[ ${#args[@]} -gt 0 ]] && cmd+=("--constructor-args" "${args[@]}")
  log "[⚙️] ${cmd[*]}"
...
# === BASE REGISTRAR SETUP (matches Forge script lines 42-46) ===
...
deploy "contracts/ethregistrar/ExponentialPremiumPriceOracle.sol" "ExponentialPremiumPriceOracle" \
  "$DummyOracle" "[0,0,20294266869609,5073566717402,158548959919]" 100000000000000000000000000 21
ExponentialPremiumPriceOracle=$(cat "$LAST_ADDR_FILE")
...
deploy "contracts/ethregistrar/ETHRegistrarController.sol" "ETHRegistrarController" \
  "$BaseRegistrarImplementation" "$ExponentialPremiumPriceOracle" 60 86400 "$ReverseRegistrar" "$DefaultReverseRegistrar" "$ENSRegistry"
...
echo "============================================" | tee -a "$LOG_FILE"
log "[✅] Full ENS stack deployment complete!"
echo "" | tee -a "$LOG_FILE"
log "[📄] Deployment addresses:"
jq . "$OUT_FILE" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
log "[📝] Full deployment log: ${LOG_FILE}"
log "[💾] Deployment JSON: ${OUT_FILE}"