...
contract WireDeployments is BaseDeployer {
    ...
    function _wireProtocolRegistryKeys(address owner, Addresses memory addr) internal {
        ...
        // `popGateway` defaults to the deploying owner for local deploys.
        // Governance rotates it post-deploy via `protocolRegistry.set`.
        registry.set(DotnsConstants.POP_GATEWAY, owner);
        vm.stopBroadcast();
