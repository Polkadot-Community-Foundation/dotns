...
Inputs: `mode` (`devnet` default or `live`), `new_owner` (the H160 of `dotns-owner`), `steps` (`all` default; see "Steps" below), `factory_nonce` (standalone steps only, default `0`), `sweep_to` (optional; required for `steps=sweep`), `confirm` (`deploy dotns to polkadot`, required for `live` whatever the step; `live` must also be dispatched from a release tag, e.g. `v0.8.0-pcf.1`, so the deployed commit keeps a permanent name), `resume` (default false; `all`/`deploy` only, see "Transient RPC failures and resume" below), `resume_run_id` (optional, with `resume`: the id of the interrupted run whose broadcasts are replayed first, see "Recovering an interrupted deploy"). Endpoints: `SUBSTRATE_WS_URL` variable, else `wss://asset-hub-paseo-rpc.n.dwellir.com` (devnet) or `wss://polkadot-asset-hub-rpc.polkadot.io` (live). Devnet uses the public ETH-RPC (`ETH_RPC_URL` variable, else `https://eth-rpc-testnet.polkadot.io`). Polkadot Asset Hub has no public ETH-RPC, so in `live` the job runs the eth-rpc adapter itself against `SUBSTRATE_WS_URL`: the pinned `docker.io/parity/eth-rpc:v1.25.0-rc2` image (pinned by digest in the workflow; `v1.24.2` could not decode live Asset Hub blocks and served null receipts, which stopped the first live run after two transactions) unless the `ETH_RPC_IMAGE` variable names another, with `--eth-pruning 4096` so the receipts of every block of the job stay served. No adapter indexes blocks from before it started: a transaction that landed in an earlier job never gets a receipt from a later one, which is why the recovery below reads Substrate instead.

The job runs these scripts, all from the repo root (which of them depends on `steps`):

| Step | Script | What it does |
| --- | --- | --- |
| Replay | `scripts/deploy/replay-broadcast.sh` | `resume=true` with `resume_run_id` only. Downloads that run's artifact (`gh run download`) and, for each stage's broadcast file with transactions lacking a receipt, in pipeline order, sends the ones that never landed at their recorded nonce and confirms each on Substrate; transactions that did land are verified, never resent. See "Recovering an interrupted deploy". |
...
**Recovering an interrupted deploy.** A run that stops with transactions broadcast but unconfirmed (the adapter returned null receipts, the job was killed, the stage retry gave up) leaves the key part way through a stage. The first live run stopped like this after the factory and `DeployCore`'s first transaction: eleven `DeployCore` transactions were never sent. The recovery is one dispatch:

1. Read the failed run: its artifact `dotns-<mode>-deploy-<run id>` holds `broadcast/<Stage>.s.sol/<chain id>/run-latest.json` per stage that ran, with `transactions` (every transaction forge planned, in nonce order; `hash` set on the ones it sent) and `receipts` (the confirmed ones). The job log's `progress:` lines give the key's nonce when it died. Nothing else is needed: forge's `--resume` is not the recovery, since the adapter of a new job never serves the receipts of blocks from before it started.
2. Dispatch the workflow again from the same tag with the same `mode`, `new_owner` and `steps` (`all` or `deploy`), `resume=true` and `resume_run_id=<the failed run's id>`. The job downloads the artifact, then runs `scripts/deploy/replay-broadcast.sh` on every stage broadcast with transactions lacking a receipt, in pipeline order (`DeployCreate3Factory`, `DeployCore`, `DeployRecords`, `DeployPolicy`, `DeployPopSystem`, `WireDeployments`). For each transaction of a file, in order: one with a receipt is skipped; one whose recorded nonce the key has already passed must have landed as ours (the block that consumed the nonce is searched for the key's `Revive.eth_transact` with that nonce, `scripts/deploy/substrate.py locate`; the hash must be the recorded one, or, for a transaction forge never sent, the target, value and calldata must be; a CREATE3 deploy must also have left code at its target, derived from the salt as `expected-set.sh` does; anything else at that nonce refuses the replay); one at the key's current nonce is sent with `cast send --async --nonce <recorded nonce>` and the recorded target, value and calldata, with the gas limit estimated at send time (`eth_estimateGas` times `REPLAY_GAS_MULTIPLIER`, 130 as forge's default: the file's `gas` is the EVM simulation's figure, and revive's real limit, weight plus storage deposit, is several times larger; an estimate that fails means the call would revert and nothing is sent), then the key's Substrate nonce is polled until it passes, and the receipt (when the adapter serves it) or the located extrinsic plus code at the target confirms it; a reverted transaction stops the replay; a recorded nonce above the key's nonce is a gap and refuses. Nothing is ever sent twice: the nonce is pinned, and a nonce that moved on is verified, not resent. The script prints one line per transaction (nonce, target, verdict) and a total.
3. The preflight then runs with `DEPLOY_RESUME=1` (factory from the chain, occupied addresses must hold the expected code and answer `owner()` with the key), and the deploy adopts: `DeployCore` finds every contract present and sends nothing (simulation only, manifest written), the later stages deploy what is missing, `WireDeployments` sets every key; then verify, and handover when `steps=all`. The summary shows the replay spend, the deploy spend and the manifest.

What the operator checks before dispatching: the key's nonce on chain equals the last recorded nonce of the failed run plus one (or the failed stage's next unsent nonce, when the last send never landed); the balance still covers what is left (`RESUME_MIN_FLOOR_DOT` floor, scaled from `MIN_BALANCE_DOT` by the empty addresses); the artifact still exists (90 days). After the run: the replay lines end with `=== Replay done: ... N sent now`, the preflight reports `occupied` equal to what landed, `DeployCore` reports `adopting without broadcasting`, and verify passes with 20 addresses. A second dispatch with the same `resume_run_id` is safe: the replay finds every transaction landed and sends nothing.

Locally, on a fork or devnet: `DEPLOY_MODE=fork|devnet SUBSTRATE_RPC_URL=... RPC_URL=... DEPLOY_SIGNER=keystore ACCOUNT_NAME=... ACCOUNT_PASSWORD=... scripts/deploy/replay-broadcast.sh broadcast/DeployCore.s.sol/<chain id>/run-latest.json` (`REPLAY_DRY_RUN=1` prints the plan and sends nothing), then `DEPLOY_RESUME=1 scripts/deploy/preflight.sh` and `DEPLOY_RESUME=1 CREATE3_FACTORY=<factory> FACTORY_NONCE=<nonce> scripts/deploy/deployall.sh`. Rehearsed on a chopsticks fork of Polkadot Asset Hub with the live run's shape (factory + one `DeployCore` transaction landed, eleven unsent, no receipts): the replay sent eleven (nonces 2 to 12, 12.35 DOT), a second replay sent none, `DeployCore` adopted with nothing sent, the remaining stages sent 54 (67 deploy transactions in all, as a clean run), the handover 14, verify passed with 20 addresses and 14 owners; 32.5 DOT from 41.5. A file whose transaction at a consumed nonce is not the one on chain, or whose deploy left no code, is refused.
